...
// Get NAFES HEI type distribution
...
router.post('/upload-csv', upload.single('file'), async (req, res) => {
  try {
    ...
    const replaceAll = req.body.replaceAll === 'true'; // Check if we should replace all data
    ...
    fs.createReadStream(filePath)
      ...
      .on('end', async () => {
        try {
          // Clear existing data if replaceAll is true
          if (replaceAll) {
            await db.query('DELETE FROM nafes_grants');
            console.log('Cleared existing NAFES grants data');
          }

          // Insert data into database
          for (const row of results) {
            // Parse budget values (remove currency symbols and commas)
