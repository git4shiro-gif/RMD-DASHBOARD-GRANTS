...
function SALIKHA() {
	...
	const [salikhaData, setSalikhaData] = useState({
	...
	const formatNumber = (value) => {
		...
		useEffect(() => {
			// Use static years for now
			setFiscalYears(['All', '2024', '2023', '2022']);
		}, []);
		...
		useEffect(() => {
			...
			setTimeout(() => {
				setSalikhaData({
					overview: { totalGrants: 9, totalAmount: 3500000, totalReleased: 2500000, activeProjects: 3, completedProjects: 6 },
					priorityArea: [
						{ name: 'Platform S', projects: 4, amount: 1500000, released: 1000000, color: '#0033a0' },
						{ name: 'Platform T', projects: 5, amount: 2000000, released: 1500000, color: '#0052cc' }
					],
					yearlyTrends: [
						{ year: 2024, projects: 3, amount: 1500000, released: 1000000 },
						{ year: 2023, projects: 3, amount: 1200000, released: 800000 },
						{ year: 2022, projects: 3, amount: 800000, released: 700000 }
					],
					regionData: [
						{ region: 'NCR', projects: 5, amount: 1800000, released: 1200000 },
						{ region: 'Region IV-A', projects: 4, amount: 1700000, released: 1300000 }
					],
					heiType: [
						{ name: 'State Universities', projects: 5, amount: 2000000, released: 1300000, color: '#0033a0' },
						{ name: 'Private HEIs', projects: 4, amount: 1500000, released: 1200000, color: '#0052cc' }
					],
					status: [
						{ name: 'Completed', value: 6, color: '#10b981' },
						{ name: 'Ongoing', value: 3, color: '#3b82f6' }
					]
				});
				setLoading(false);
			}, 500);
	...
	const handleFileUpload = async (file, replaceAll = true) => {
		...
		try {
			const response = await fetch(`${API_BASE_URL}/api/salikha/upload-csv`, { method: 'POST', body: formData });
		...
		return (
			<main className='main-container' style={{ padding: '20px', background: 'linear-gradient(135deg, #ffffff 0%, #f8fafc 50%, #f1f5f9 100%)', minHeight: '100vh', display: 'flex', justifyContent: 'center', alignItems: 'center' }}>
				<div style={{ textAlign: 'center' }}>
					...
					<h2 style={{ color: '#0033a0' }}>Loading SALIKHA data...</h2>
				</div>
				<style>{`
					@keyframes spin {
			...
			<div style={{ maxWidth: '1800px', margin: '0 auto' }}>
				...
				<header style={{ marginBottom: '32px', paddingBottom: '24px', borderBottom: '3px solid rgba(0, 51, 160, 0.08)' }}>
					...
					<p style={{ color: '#64748b', fontSize: '0.95rem', margin: '0', lineHeight: '1.5' }}>
						SALIKHA research grants support development and innovation initiatives in higher education institutions.
					</p>
				</header>
				{/* Image Slider */}
				...
				<div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: '24px', marginBottom: '48px' }}>
					{[
						{ title: 'Total Projects', subtitle: 'All SALIKHA grants', value: formatNumber(salikhaData.overview.totalGrants), icon: <FaFolderOpen />, color: '#3b82f6', bgColor: 'rgba(59, 130, 246, 0.1)' },
						{ title: 'Total Amount', subtitle: 'Total grant amount', value: formatAmount(salikhaData.overview.totalAmount), icon: <FaMoneyBillWave />, color: '#10b981', bgColor: 'rgba(16, 185, 129, 0.1)' },
						{ title: 'Total Released', subtitle: 'Funds released', value: formatAmount(salikhaData.overview.totalReleased), icon: <FaHandHoldingUsd />, color: '#f59e0b', bgColor: 'rgba(245, 158, 11, 0.1)' },
						{ title: 'Active Projects', subtitle: 'Ongoing', value: formatNumber(salikhaData.overview.activeProjects), icon: <FaSpinner />, color: '#6366f1', bgColor: 'rgba(99, 102, 241, 0.1)' },
						{ title: 'Completed Projects', subtitle: 'Finished', value: formatNumber(salikhaData.overview.completedProjects), icon: <FaCheckCircle />, color: '#10b981', bgColor: 'rgba(16, 185, 129, 0.1)' }
					].map((card, idx) => (
