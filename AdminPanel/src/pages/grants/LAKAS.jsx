...
function LAKAS() {
	...
	const [lakasData, setLakasData] = useState({
	...
	const formatNumber = (value) => {
		...
		useEffect(() => {
			...
			setTimeout(() => {
				setLakasData({
					overview: { totalGrants: 12, totalAmount: 5000000, totalReleased: 3500000, activeProjects: 4, completedProjects: 8 },
					priorityArea: [
						{ name: 'Platform A', projects: 5, amount: 2000000, released: 1500000, color: '#0033a0' },
						{ name: 'Platform B', projects: 7, amount: 3000000, released: 2000000, color: '#0052cc' },
						{ name: 'Private HEIs', projects: 5, amount: 2000000, released: 1500000, color: '#0052cc' }
					],
					status: [
	...
	const handleFileUpload = async (file, replaceAll = true) => {
		...
		try {
			const response = await fetch(`${API_BASE_URL}/api/lakas/upload-csv`, { method: 'POST', body: formData });
		...
		return (
			<main className='main-container' style={{ padding: '20px', background: 'linear-gradient(135deg, #ffffff 0%, #f8fafc 50%, #f1f5f9 100%)', minHeight: '100vh', display: 'flex', justifyContent: 'center', alignItems: 'center' }}>
				<div style={{ textAlign: 'center' }}>
					...
					<h2 style={{ color: '#0033a0' }}>Loading LAKAS data...</h2>
				</div>
				<style>{`
					@keyframes spin {
		...
		<main className="main-container">
			...
			<div style={{ maxWidth: '1800px', margin: '0 auto' }}>
				...
				<div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: '24px', marginBottom: '48px' }}>
					{[
						{ title: 'Total Projects', subtitle: 'All LAKAS grants', value: formatNumber(lakasData.overview.totalGrants), icon: <FaFolderOpen />, color: '#3b82f6', bgColor: 'rgba(59, 130, 246, 0.1)' },
						{ title: 'Total Amount', subtitle: 'Total grant amount', value: formatAmount(lakasData.overview.totalAmount), icon: <FaMoneyBillWave />, color: '#10b981', bgColor: 'rgba(16, 185, 129, 0.1)' },
						{ title: 'Total Released', subtitle: 'Funds released', value: formatAmount(lakasData.overview.totalReleased), icon: <FaHandHoldingUsd />, color: '#f59e0b', bgColor: 'rgba(245, 158, 11, 0.1)' },
						{ title: 'Active Projects', subtitle: 'Ongoing', value: formatNumber(lakasData.overview.activeProjects), icon: <FaSpinner />, color: '#6366f1', bgColor: 'rgba(99, 102, 241, 0.1)' },
						{ title: 'Completed Projects', subtitle: 'Finished', value: formatNumber(lakasData.overview.completedProjects), icon: <FaCheckCircle />, color: '#10b981', bgColor: 'rgba(16, 185, 129, 0.1)' }
					].map((card, idx) => (
