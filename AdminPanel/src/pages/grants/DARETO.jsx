...
function DARETO() {
  ...
  const [daretoData, setDaretoData] = useState({
  ...
  useEffect(() => {
    ...
    setTimeout(() => {
      setDaretoData({
        overview: { totalGrants: 10, totalAmount: 4000000, totalReleased: 3000000, activeProjects: 3, completedProjects: 7 },
        priorityArea: [
          { name: 'Platform X', projects: 4, amount: 1500000, released: 1000000, color: '#0033a0' },
          { name: 'Platform Y', projects: 6, amount: 2500000, released: 2000000, color: '#0052cc' }
        ],
        yearlyTrends: [
          { year: 2024, projects: 3, amount: 1500000, released: 1000000 },
          { year: 2023, projects: 4, amount: 1300000, released: 900000 },
          { year: 2022, projects: 3, amount: 1200000, released: 1100000 }
        ],
        regionData: [
          { region: 'NCR', projects: 5, amount: 2000000, released: 1500000 },
          { region: 'Region IV-A', projects: 5, amount: 2000000, released: 1500000 }
        ],
        heiType: [
          { name: 'State Universities', projects: 6, amount: 2500000, released: 1800000, color: '#0033a0' },
  ...
  const handleFileUpload = async (file, replaceAll = true) => {
    ...
    try {
      const response = await fetch(`${API_BASE_URL}/api/dareto/upload-csv`, { method: 'POST', body: formData });
    ...
    return (
      <main className='main-container' style={{ padding: '20px', background: 'linear-gradient(135deg, #ffffff 0%, #f8fafc 50%, #f1f5f9 100%)', minHeight: '100vh', display: 'flex', justifyContent: 'center', alignItems: 'center' }}>
        <div style={{ textAlign: 'center' }}>
          ...
          <h2 style={{ color: '#0033a0' }}>Loading DARETO data...</h2>
        </div>
        <style>{`
          @keyframes spin {
      ...
      <div style={{ maxWidth: '1800px', margin: '0 auto' }}>
        ...
        <header style={{ marginBottom: '32px', paddingBottom: '24px', borderBottom: '3px solid rgba(0, 51, 160, 0.08)' }}>
          ...
          <p style={{ color: '#64748b', fontSize: '0.95rem', margin: '0', lineHeight: '1.5' }}>
            DARETO research grants support development and innovation initiatives in higher education institutions.
          </p>
        </header>
        {/* Image Slider */}
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: '24px', marginBottom: '48px' }}>
          {[
            { title: 'Total Projects', subtitle: 'All DARETO grants', value: formatNumber(daretoData.overview.totalGrants), icon: <FaFolderOpen />, color: '#3b82f6', bgColor: 'rgba(59, 130, 246, 0.1)' },
            { title: 'Total Amount', subtitle: 'Total grant amount', value: formatAmount(daretoData.overview.totalAmount), icon: <FaMoneyBillWave />, color: '#10b981', bgColor: 'rgba(16, 185, 129, 0.1)' },
            { title: 'Total Released', subtitle: 'Funds released', value: formatAmount(daretoData.overview.totalReleased), icon: <FaHandHoldingUsd />, color: '#f59e0b', bgColor: 'rgba(245, 158, 11, 0.1)' },
            { title: 'Active Projects', subtitle: 'Ongoing', value: formatNumber(daretoData.overview.activeProjects), icon: <FaSpinner />, color: '#6366f1', bgColor: 'rgba(99, 102, 241, 0.1)' },
            { title: 'Completed Projects', subtitle: 'Finished', value: formatNumber(daretoData.overview.completedProjects), icon: <FaCheckCircle />, color: '#10b981', bgColor: 'rgba(16, 185, 129, 0.1)' }
          ].map((card, idx) => (
