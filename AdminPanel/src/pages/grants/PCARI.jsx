...
function PCARI() {
	...
	const [pcariData, setPcariData] = useState({
	...
	const formatNumber = (value) => {
		...
		useEffect(() => {
			...
			setTimeout(() => {
				setPcariData({
					overview: { totalGrants: 8, totalAmount: 3000000, totalReleased: 2000000, activeProjects: 2, completedProjects: 6 },
					priorityArea: [
						{ name: 'Platform P', projects: 3, amount: 1000000, released: 700000, color: '#0033a0' },
						{ name: 'Platform Q', projects: 5, amount: 2000000, released: 1300000, color: '#0052cc' }
					],
					yearlyTrends: [
						{ year: 2024, projects: 2, amount: 1000000, released: 700000 },
						{ year: 2023, projects: 3, amount: 1200000, released: 800000 },
						{ year: 2022, projects: 3, amount: 800000, released: 500000 }
					],
					regionData: [
						{ region: 'NCR', projects: 4, amount: 1500000, released: 1000000 },
						{ region: 'Region IV-A', projects: 4, amount: 1500000, released: 1000000 }
					],
					heiType: [
						{ name: 'State Universities', projects: 5, amount: 2000000, released: 1300000, color: '#0033a0' },
						{ name: 'Private HEIs', projects: 3, amount: 1000000, released: 700000, color: '#0052cc' }
					],
					status: [
	...
	const handleFileUpload = async (file, replaceAll = true) => {
		...
		try {
			const response = await fetch(`${API_BASE_URL}/api/pcari/upload-csv`, { method: 'POST', body: formData });
		...
		return (
			<main className='main-container' style={{ padding: '20px', background: 'linear-gradient(135deg, #ffffff 0%, #f8fafc 50%, #f1f5f9 100%)', minHeight: '100vh', display: 'flex', justifyContent: 'center', alignItems: 'center' }}>
				<div style={{ textAlign: 'center' }}>
					...
					<h2 style={{ color: '#0033a0' }}>Loading PCARI data...</h2>
				</div>
				<style>{`
					@keyframes spin {
		...
		<main className="main-container">
			...
			<div style={{ maxWidth: '1800px', margin: '0 auto' }}>
				...
				<div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: '24px', marginBottom: '48px' }}>
					{[
						{ title: 'Total Projects', subtitle: 'All PCARI grants', value: formatNumber(pcariData.overview.totalGrants), icon: <FaFolderOpen />, color: '#3b82f6', bgColor: 'rgba(59, 130, 246, 0.1)' },
						{ title: 'Total Amount', subtitle: 'Total grant amount', value: formatAmount(pcariData.overview.totalAmount), icon: <FaMoneyBillWave />, color: '#10b981', bgColor: 'rgba(16, 185, 129, 0.1)' },
						{ title: 'Total Released', subtitle: 'Funds released', value: formatAmount(pcariData.overview.totalReleased), icon: <FaHandHoldingUsd />, color: '#f59e0b', bgColor: 'rgba(245, 158, 11, 0.1)' },
						{ title: 'Active Projects', subtitle: 'Ongoing', value: formatNumber(pcariData.overview.activeProjects), icon: <FaSpinner />, color: '#6366f1', bgColor: 'rgba(99, 102, 241, 0.1)' },
						{ title: 'Completed Projects', subtitle: 'Finished', value: formatNumber(pcariData.overview.completedProjects), icon: <FaCheckCircle />, color: '#10b981', bgColor: 'rgba(16, 185, 129, 0.1)' }
					].map((card, idx) => (
