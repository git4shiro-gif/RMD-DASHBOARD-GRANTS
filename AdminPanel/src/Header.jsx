    ...
    <header className="header">
      <div className="header-row-flex">
        <div className="header-left-mobile-menu">
          <button className="menu-icon" aria-label="Open navigation" onClick={() => setSidebarOpen(true)}>
            <span className="icon-bar"></span>
            <span className="icon-bar"></span>
            <span className="icon-bar"></span>
          </button>
        </div>
        {/* Desktop nav remains as before */}
        <nav className="main-nav" style={{ display: 'flex', alignItems: 'center', justifyContent: 'flex-start', gap: 0, flex: 1 }}>
          ...
          <a
            ...
            onClick={() => { setMobileNavOpen(false); setOpenDropdown(null); }}
              style={{ marginLeft: 24, fontWeight: 700, color: '#222' }}
          >
      ...
      {sidebarOpen && (
        <>
          ...
          <nav className="sidebar-nav">
            ...
            <button className="nav-btn" onClick={() => { handleHomeClick(); setSidebarOpen(false); }}>Home</button>
            <a href="https://www.gov.ph/" target="_blank" rel="noopener noreferrer" className="ched-brand" style={{ fontWeight: 700, color: '#0033a0', textDecoration: 'none', fontSize: '1rem', letterSpacing: '1px', display: 'flex', alignItems: 'center', height: '40px' }}>
              GOVPH
            </a>
            <a
            ...
            <button className="nav-btn" onClick={() => { setAboutModalOpen(true); setSidebarOpen(false); }}>About Us</button>
            <div className="nav-dropdown">
              <button className="nav-btn" onClick={() => setOpenDropdown(openDropdown === 'announcements' ? null : 'announcements')} aria-expanded={openDropdown === 'announcements'}>
                Announcements ▾
              </button>
              {openDropdown === 'announcements' && (
                <div className="dropdown-menu">
                  <a href="#" className="dropdown-item" onClick={() => setSidebarOpen(false)}>Information</a>
                  <a href="#" className="dropdown-item" onClick={() => setSidebarOpen(false)}>Procurement</a>
                  <button className="dropdown-item" onClick={() => { setSidebarOpen(false); navigate('/fullcalendar'); }}>Events (Full Calendar)</button>
                </div>
              )}
            </div>
            <div className="nav-dropdown">
              <button className="nav-btn" onClick={() => setOpenDropdown(openDropdown === 'grants' ? null : 'grants')} aria-expanded={openDropdown === 'grants'}>
                Grants ▾
              </button>
              {openDropdown === 'grants' && (
                <div className="dropdown-menu">
                  <button className="dropdown-item" onClick={() => { setSidebarOpen(false); navigate('/grants/gia'); }}>GIA</button>
                  <button className="dropdown-item" onClick={() => { setSidebarOpen(false); navigate('/grants/idig'); }}>IDIG</button>
                  <button className="dropdown-item" onClick={() => { setSidebarOpen(false); navigate('/grants/lakas'); }}>LAKAS</button>
                  <button className="dropdown-item" onClick={() => { setSidebarOpen(false); navigate('/grants/pcari'); }}>PCARI</button>
                  <button className="dropdown-item" onClick={() => { setSidebarOpen(false); navigate('/grants/salikha'); }}>SALIKHA</button>
                  <button className="dropdown-item" onClick={() => { setSidebarOpen(false); navigate('/grants/dareto'); }}>DARETO</button>
                  <button className="dropdown-item" onClick={() => { setSidebarOpen(false); navigate('/grants/nafes'); }}>NAFES</button>
                </div>
              )}
            </div>
          </nav>
